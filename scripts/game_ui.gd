...
var tone_to_expression: Dictionary[String, CharaExpression] = {
...
@onready var hand_cards: Array[Control] = [
...
func _ready() -> void:
  for i in range(hand_cards.size()):
    ...
    if card.has_signal("gui_input"):
      var right_click_callable: Callable = Callable(self, "_on_card_gui_input").bind(i)
...
func _check_for_game_end() -> void:
  ...
  if GameManager.time_left <= 0.0:
    ...
    if GameManager.trust_vs_confusion >= 70.0 and GameManager.hype_vs_meme >= 70.0:
      _trigger_ending(
        "THE AWESOME ENDING",
    ...
    # high hype low trust
    elif GameManager.trust_vs_confusion < 30.0 and GameManager.hype_vs_meme >= 60.0:
      _trigger_ending(
        "THE CHAOS ENDING",
...
func _update_ui_display() -> void:
  ...
  if GameManager.awaiting_round_continue:
    var feedback_text: String = "ROUND SCORE: " + str(int(round(GameManager.last_round_score))) + "\n"
  ...
  var lock_inputs: bool = GameManager.awaiting_round_continue
  reporter_bubble.visible = not lock_inputs
  for i in range(hand_cards.size()):
    if i < GameManager.current_hand.size():
      var card_data: WordData = GameManager.current_hand[i]
      var card: Control = hand_cards[i]

      if card.has_method("setup_card_data"):
        card.setup_card_data(card_data, i)

      var is_selected: bool = GameManager.selected_speech_line.has(card_data)
      card.visible = true
      var category_color: Color = _get_category_color(card_data.category)
      var alpha: float = 0.35 if is_selected else 1.0
...
func _tween_trust_bar(target_value: float) -> void:
  var clamped_target: float = clamp(target_value, trust_bar.min_value, trust_bar.max_value)
...
func _tween_vibe_bar(target_value: float) -> void:
  var clamped_target: float = clamp(target_value, vibe_bar.min_value, vibe_bar.max_value)
...
func _tween_time_bar(target_value: float) -> void:
  var clamped_target: float = clamp(target_value, time_bar.min_value, time_bar.max_value)
...
func _on_spin_pressed() -> void:
  ...
  var available_indices: Array[int] = _get_available_spin_indices()
...
func _spin_into_card_index(target_index: int) -> void:
  ...
  var selected_word: WordData = await _play_spinner_animation()
  if selected_word:
...
func _on_deliver_or_continue_pressed() -> void:
  if GameManager.awaiting_round_continue:
    ...
    var submitted_speech: String = GameManager.build_sentence(GameManager.selected_speech_line)
    GameManager._evaluate_selected_line()
    if GameManager.awaiting_round_continue:
      var completed_speech: String = GameManager.last_round_speech_text
...
func _update_total_score_label(play_pulse: bool) -> void:
  ...
  var rounded_total: int = int(round(GameManager.total_score))
...
func _show_round_feedback_popups(submitted_words: Array[WordData], feedback: Dictionary) -> void:
  ...
  var word_count: int = min(word_scores.size(), submitted_words.size())
  for i in range(word_count):
    var word_entry: Dictionary = word_scores[i]
    var word_ref: WordData = submitted_words[i]
    var hand_index: int = GameManager.current_hand.find(word_ref)
    var popup_origin: Vector2 = preview_lbl.get_global_rect().get_center() + Vector2(0.0, -80.0 * feedback_ui_scale)
    ...
    var word_impact: float = float(word_entry.get("impact", 0.0))
  ...
  for i in range(combo_entries.size()):
    ...
    var combo_impact: float = float(combo_entry.get("impact", 0.0))
    var combo_name: String = String(combo_entry.get("name", "combo"))
    var combo_text: String = "COMBO: " + combo_name.to_upper() + " " + _format_signed_number(combo_impact)
    var combo_origin: Vector2 = question_lbl.get_global_rect().get_center() + Vector2(0.0, (-180.0 * feedback_ui_scale) - ((72.0 * feedback_ui_scale) * i))
    _spawn_floating_feedback(combo_text, combo_origin, combo_impact >= 0.0, 0.2 + (float(i) * 0.08), int(round(70.0 * feedback_ui_scale)))

  var round_score: float = float(feedback.get("score", GameManager.last_round_score))
  var round_text: String = "ROUND " + _format_signed_number(round_score)
...
func _spawn_floating_feedback(text: String, global_origin: Vector2, is_positive: bool, delay: float, font_size: int) -> void:
  ...
  var target_position: Vector2 = popup.position + Vector2(0.0, -feedback_float_distance)
  var popup_tween: Tween = create_tween().set_parallel(true)
...
func _spawn_crowd_reactions(feedback: Dictionary) -> void:
  ...
  var combo_entries: Array = feedback.get("combo_entries", [])
  var combo_impact: float = 0.0
  for combo_entry: Dictionary in combo_entries:
    combo_impact += float(combo_entry.get("impact", 0.0))

  var round_score: float = float(feedback.get("score", GameManager.last_round_score))
  var sentiment: float = round_score + (combo_impact * 0.8)
  var texture_pool: Array[Texture2D] = reaction_neutral_textures
  if sentiment >= 45.0:
  ...
  var crowd_rect: Rect2 = crowd.get_global_rect()
  var spawn_area_position: Vector2 = crowd_rect.position + Vector2(crowd_rect.size.x * 0.12, crowd_rect.size.y * 0.12)
  var spawn_area_size: Vector2 = Vector2(crowd_rect.size.x * 0.76, crowd_rect.size.y * 0.42)

  for i in range(spawn_count):
    var reaction: TextureRect = TextureRect.new()
    ...
    var texture_size: Vector2 = reaction.texture.get_size()
    reaction.custom_minimum_size = texture_size
    reaction.size = texture_size

    var spawn_position: Vector2 = spawn_area_position + Vector2(
    ...
    var reaction_scale: float = randf_range(crowd_reaction_scale_range.x, crowd_reaction_scale_range.y)
    reaction.scale = Vector2(reaction_scale * 0.75, reaction_scale * 0.75)
    reaction.modulate.a = 0.0

    var reaction_tween: Tween = create_tween().set_parallel(true)
    var delay: float = crowd_reaction_spawn_delay_step * float(i)
...
func _play_spinner_animation() -> WordData:
  ...
  var extra_turns: int = randi_range(5, 8)
  var final_degrees: float = float(extra_turns * 360 + randi_range(0, 359))
  var spin_duration: float = randf_range(1.8, 2.6)
  var spin_tween: Tween = create_tween().set_trans(Tween.TRANS_CUBIC).set_ease(Tween.EASE_OUT)
  spin_tween.tween_property(wheel_bg, "rotation_degrees", final_degrees, spin_duration)
  await spin_tween.finished

  var selected_word: WordData = _get_top_spinner_word()
...
func _update_politician_expression_for_tone(tone: String) -> void:
  var normalized_tone: String = tone.to_lower()
...
func _get_top_spinner_word() -> WordData:
  ...
  for i in range(spinner_word_labels.size()):
    var label: Label = spinner_word_labels[i]
...
func _play_crowd_bob() -> void:
  ...
  var bob_amount: float = randf_range(crowd_bob_amount_range.x, crowd_bob_amount_range.y)
  var bob_duration: float = randf_range(crowd_bob_duration_range.x, crowd_bob_duration_range.y)
